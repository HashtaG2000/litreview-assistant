"""
Evaluation layer for LitMap.

Provides:
  - Semantic chunk ranking (Fix 1): rank a candidate paper's own chunks against
    the research idea rather than blindly taking the first N.
  - Retrieval quality metrics (Fix 2): per-paper confidence scores and an overall
    review quality score surfaced in the UI.
"""
...
# Module-level embedder to avoid reloading the model on every call
_embedder = None
...
def _scale_to_pct(cosine_score: float) -> int:
    ...
    Typical MiniLM similarity for related academic text sits in [0.2, 0.85],
    so we scale that range to fill 0–100 %.
    """
...
# ── Fix 1: semantic chunk selection ──────────────────────────────────────────
...
def get_top_chunks_by_similarity(research_idea: str, chunks: list, k: int = 6):
    ...
    This replaces the naive `chunks[:5]` approach — key contributions may appear
    in the abstract, introduction, or conclusion, not just the first pages.
    ...
    confidence = _scale_to_pct(avg_score)

    return top_chunks, confidence


# ── Fix 2: retrieval quality metrics ─────────────────────────────────────────

def compute_review_quality(research_idea: str, paper_titles: list, search_fn, k_per_paper: int = 4):
    """
    Compute per-paper retrieval confidence for the Review Quality panel.

    Args:
        research_idea  – user's research idea string
        paper_titles   – list of accepted paper names
        search_fn      – callable(query, paper_title, k) -> list[Document]
                         (should be retriever.search_similar_filtered)
        k_per_paper    – chunks to retrieve per paper

    Returns:
        paper_stats    – dict[paper_title] = {"chunks_retrieved": int, "confidence": int}
